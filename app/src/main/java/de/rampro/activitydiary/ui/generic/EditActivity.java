            ...
            if ((cursor != null)) {
                if(token == SIMILAR_ACTIVITY) {
                    ...
                    if(cursor.moveToFirst()) {
                        ...
                        @SuppressLint("Range") boolean deleted = (cursor.getLong(cursor.getColumnIndex(ActivityDiaryContract.DiaryActivity._DELETED)) != 0);
                        @SuppressLint("Range") int actId = cursor.getInt(cursor.getColumnIndex(ActivityDiaryContract.DiaryActivity._ID));
                        @SuppressLint("Range") String name = cursor.getString(cursor.getColumnIndex(ActivityDiaryContract.DiaryActivity.NAME));
